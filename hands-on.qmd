...
How many coffee shops can Annika reach from her coworking space? (cumulative opportunities accessibility)

## Task 2 ⭐⭐⚪

Now integrate a distance decay (gravitiy accessibility)

## Task 3 ⭐⭐⚪

Also integrate a attractiveness of the different destinations

## Task 4 ⭐⭐⭐

Integrate both supply and demand into the measure (2SFCA accessibility)
:::
...
Create a figure with the street directions/azimuths ⭐⚪⚪
https://geoffboeing.com/2019/09/urban-street-network-orientation/

```{r}
#| code-fold: true
#| code-summary: "Unfold me for..."
ct_azimuths = ct_sfn_clean |> 
  morph(to_spatial_transformed, 4326) |> 
  activate(edges) |> 
  mutate(angle = edge_azimuth(degrees = TRUE)) |> 
  unmorph()
```
...
Plot and map the distributions of degree centralilty, betweenness centrality, straightness centrality ⭐⭐⚪

## Task 3 ⭐⭐⭐

Are there any difference in the network orientation per suburb? 

```{r}
#| code-fold: true
#| code-summary: "Unfold me for..."
# Project suburb data and wrangle
ct_suburbs = st_transform(ct_suburbs, st_crs(ct_azimuths)) |> 
  transmute(suburb = name)
ct_azimuths_suburb = st_join(ct_azimuths, ct_suburbs)
```
...
#| code-summary: "Unfold me for..."
library(units)
ggplot(st_as_sf(ct_azimuths_suburb, "edges")) +
  geom_histogram(aes(drop_units(angle)), bins = 72) +
  scale_x_continuous(
    limits = c(-180, 180),
    expand = c(0,0),
  ...
  coord_radial(start = pi, direction = 1) +
  facet_wrap(~suburb, scales = "free_y") +
  theme_bw() +
  theme(
    axis.title = element_blank(),
    axis.ticks = element_blank(),
...
Compute the probabilities of lion sightings per road type

## Task 2 ⭐⭐⚪

Compute the most optimal route so that Francois can visit all the camps and how long would it take him?

## Task 3 ⭐⭐⚪

Identify locations with high probabilities of seeing the big five, do these correspond to the rest camps?
:::
...
Create a map of animal sightings delineating hotspots

## Task 2 ⭐⭐⚪

Compute some descriptive statistics of the sightings in relation to the network

## Task 3 ⭐⭐⭐

Can Amina use Kruger data to predict wildlife sightings in the Limpopo Park in Mozambique?
:::
